...
interface BEIFormProps {
  orderType: string;
  orderOption: string;
}

export default function BEIForm({ orderType, orderOption }: BEIFormProps) {
  ...
  const { mutate: onCreateProject } = useMutation({
    ...
    onSuccess: (result: ProjectType) => {
      ...
      if (orderOption === "manual-calculation") {
        router.push(`/manual-calculation/${result.id}`);
      } else {
        router.push(`/drawing/${result.id}`);
      }
    },
    onError: (error) => {
  ...
  const form = useForm<z.infer<typeof orderSchema>>({
    ...
    defaultValues: {
      type: "build-electrical-installation",
      level: orderLevel,
      name: "",
      phone_number: "",
      address: "",
        ...
        <div className="flex-[1] space-y-3 rounded-xl bg-white p-2 lg:p-3">
          <div className="w-full items-center space-y-4 rounded-lg border-2 border-dashed p-4 lg:space-y-6 lg:p-6">
            ...
            <div className="lg:flex">
              ...
              <p className="w-64">
                Build Electrical Installation - {orderLevel}
              </p>
            </div>
            <Separator />
            <div className="lg:flex">
